...
/*
 * student_information_mangement_system.c
 *
 *  Created on: ??þ/??þ/????
 *      Author: HP
 */
...
void Find_The_Student_By_First_Name(FIFO_t* FIFO_Buf)
{
	...
	//is database Full
	else if(FIFO_Buf_Is_Full(FIFO_Buf) == FIFO_Full)
	{
		printf("[ERROR] DataBase is Full\n");
	}
	//is database Empty
	else if(FIFO_Buf->head == FIFO_Buf->base )
	{
...
void Count_Of_Students(FIFO_t* FIFO_Buf)
{
	...
	/***************************WATCH OUT****************************/
	/****************************************************************/
	//i cant replace a this implemtation with my Gloabal variable "Student_Number" that is already written up here
	printf("-----------------------\n");
	//is database already Exit
	if(!FIFO_Buf->base || !FIFO_Buf->tail ||!FIFO_Buf->head)
	{
...
{   
	...
	if(!FIFO_Buf->base || !FIFO_Buf->tail ||!FIFO_Buf->head)
	{
	 ...
	 //is Roll exist already in database
	//is database Empty
	else if(FIFO_Buf->head == FIFO_Buf->base )
	{
		printf("[ERROR] No Student in DataBase\n");
